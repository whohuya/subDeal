import { query, queryAll,select, add, update, remove } from '../../services/teachers/majorClass'

export default {
  namespace: 'teachers/majorClass',
  ...
  effects: {
    ...
    * select ({payload,callback}, {call, put}) {
      ...
      // yield put({type: 'saveSelected', payload: response})
      if (callback) {
        callback(response)
      }
    },
  ...
  reducers: {
    ...
    saveAll (state, action) {
      return {
        ...state,
        allMajorClasses: [...action.payload]
      }
    },
    saveSelected (state, action) {
      return {
        ...state,
        selectedMajorClasses: {...action.payload}
      }
    },
